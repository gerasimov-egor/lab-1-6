...
public class Lab3_1 {
    public static void main(String[] args) {
        ...
        try {
            while (true) {
                ...
                System.out.print("Введите элемент меньше которого должны быть числа ");
                ...
                for (int i = 0; i < row_count; i++) {
                    for (int j = 0; j < column_count; j++) {
                        int tmp[] = new int[2];
                        if (array[i][j] < item_to_search && Math.abs(i-j) % 5 == 0) {
                            count += 1;
                            total_array.add(i);
                            total_array.add(j);
                        }
    ...
    private static void output_vector(int item_to_search, int count, ArrayList<Integer> total_array){
        System.out.println("Количество элементов меньше числа " + item_to_search + "И модуль разности которых кратен 5 в массиве равно " + count);
